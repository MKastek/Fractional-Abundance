...
class FA_GUI(tk.Tk):
    def __init__(self):
        ...
        self.ACD_button = Button(
            self.top_frame, text="ACD", command=self.open_ACD_file, width=10, height=2
        )
        self.SCD_button = Button(
            self.top_frame, text="SCD", command=self.open_SCD_file, width=10, height=2
        )
        self.plot_button = Button(
            self.top_frame,
            text="plot",
            command=lambda: threading.Thread(target=self.plot).start(),
            width=10,
            height=2,
        )
        self.save_button = Button(
            self.top_frame, text="save", command=self.save, width=10, height=2
        )
        ...
        self.slider_Te = Scale(
            self.slider_frame_Te,
            orient="horizontal",
            from_=1.1,
            to=4.0,
            resolution=0.1,
            variable=self.Te_scale,
            command=lambda x: self.replot(),
        )
    ...
    def plot_data(self, scale_Te=4.0):
        ...
        self.annot = self.subplot.annotate(
            "", xy=(300, 0.01), xytext=(0, 0), textcoords="offset points"
        )
    ...
    def on_plot_hover(self, event):
        ...
        for curve, num in zip(
            self.subplot.get_lines(), range(len(self.subplot.get_lines()))
        ):
