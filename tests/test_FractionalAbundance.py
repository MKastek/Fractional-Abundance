...
class TestFA(unittest.TestCase):
    ...
    def plot_result(time_df):
        time_df.drop('charge-number', axis=1).plot.bar(rot=0, title='Computation time [s]').grid()
        ...
        plt.scatter(time_df['charge-number'], time_df['multi-threading-numba'], label='numba with threading')
        plt.plot(time_df['charge-number'], time_df['multi-threading-numba'], linestyle="--")
        plt.minorticks_on()
    ...
    @classmethod
    def setUpClass(cls):
        cls.element_list = ['He', 'Li', 'Ne', 'Ar', 'Kr', 'Mo', 'Xe', 'B', 'Al', 'Cr', 'O']
    ...
    def test_concurrent(self):
        ...
        for i, element in enumerate(TestFA.element_list):
            with self.subTest(i=i):
                ...
                t1 = timeit(stmt="FractionalAbundance(element, True)",
                           globals={'FractionalAbundance': FractionalAbundance, 'element': element}, number=trials) / trials

                t2 = timeit(stmt="FractionalAbundance(element, False)",
                           globals={'FractionalAbundance': FractionalAbundance, 'element': element}, number=trials) / trials
